...
const ChaptersList = () => {
  ...
  const { classId:initialClassId, subjectId:initialSubectId, chapterId:initialChapterId }=location.state||{}
    ...
    <div className="w-full overflow-x-auto">
      <table className="w-full bg-white shadow-md rounded-lg">
        <thead className="bg-gradient-to-r from-blue-400 to-purple-600 text-white">
          <tr>
            <th className="px-6 py-4 text-sm font-semibold text-left min-w-[150px]">Chapter Title</th>
            <th className="px-6 py-4 text-sm font-semibold text-left min-w-[200px]">Description</th>
            <th className="px-6 py-4 text-sm font-semibold text-left min-w-[120px]">Action</th>
          </tr>
        </thead>
        <tbody>
          {filteredChapters.length > 0 ? (
            filteredChapters.map((chapter) => (
              <tr key={chapter._id} className="border-b hover:bg-gray-100 transition duration-200">
                <td className="px-6 py-4 text-lg font-medium">{chapter.title}</td>
                <td className="px-6 py-4">{chapter.description || "No description available"}</td>
                <td className="px-6 py-4 flex gap-3 justify-center">
                  ...
                  {/* Notes Button */}
                  <button
                    onClick={() => navigate("/teacher-dashboard/chapterCurrs", {
                      state: {
                  ...
                  {/* Update Button */}
                  <button
                    onClick={() => navigate("/teacher-dashboard/update_chapter", {
                      state: { chapter },
                    })}
            ...
            <tr>
              <td colSpan="3" className="px-6 py-4 text-center text-gray-600">No chapters found</td>
            </tr>
          )}
        </tbody>
      </table>
